...
module.exports = [
    {
        ...
        handler: async (request, h) => {
            ...
            try {
                ...
                await emailService.sendEmailToNewUser(email, firstName, lastName);

                // Créer l'utilisateur dans la base de données
                const newUser = await userService.create(request.payload);

                return h.response(newUser).code(201); // 201 Created
            } catch (error) {
                // Gestion des erreurs
                ...
                return Boom.badImplementation('Une erreur est survenue lors de la création du compte', error);
            }
        }
    },
    {
